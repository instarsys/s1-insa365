...
| 62개 API Route | ✅ ~95% | 대부분 실제 DB 연동, 2개 시스템 페이지만 mock |
| 38개 페이지 UI + 6 Super Admin | ✅ ~90% | 대부분 SWR 훅 연결됨, Super Admin 콘솔 분리 완료 |
| JWT 인증 + PII 암호화 | ✅ 완료 | |
| DB 실행 + Seed | ❌ 미실행 | Docker + migrate + seed 필요 |
| PWA manifest | ❌ 없음 | |
...
### Sprint 1: Foundation — DB 실행 + 데이터 파이프라인 연결 (Week 1)
...
### Sprint 2: Payroll End-to-End — 입력·편집·확정·조회 (Week 2)
...
### Sprint 3: Attendance + GPS + Leave (Week 3)
...
### Sprint 5: Employee Mobile PWA + Notifications (Week 5)
...
### Sprint 6: Polish + Validation + QA + Launch Prep (Week 6)
...
1. **Zod 입력 검증** (핵심 10개 API)
   ...
   - `src/presentation/api/schemas/` 디렉토리에 공유 스키마

2. **데모용 시드 50명 확충**
   - `prisma/seed.ts` — 50명 직원 + 2개월 근태 + 1개월 급여 데이터
   - 부서별 분산, 다양한 급여 유형 (고정급/시급/일급)
...
5. **E2E 스모크 테스트** (수동 또는 Playwright)
   - 관리자: 가입 → 직원등록 → 근태입력 → 근태확정 → 급여계산 → 확정 → 대장조회
   - 직원: 출퇴근 → 휴가신청 → 명세서 확인
...
**검증:** 전체 워크플로우 E2E 통과, Go/No-Go 6개 항목 전부 통과
...
- Sprint 6: 50명 데이터 Go/No-Go 체크리스트 전항 통과

### Go/No-Go 기준 (MVP.md §12)
1. 급여 계산 정확도 99%+ (10개사 데이터 검증)
2. 셀프 온보딩 3분 이내
3. 50명 급여 일괄 계산 10초 이내
4. 4대보험 4종 + 소득세 2종 자동계산 정상
5. 52시간 초과 감지 + 경고
6. 보안 감사 (PII 암호화, 인증, RBAC)
