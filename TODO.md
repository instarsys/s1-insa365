...
> 최종 업데이트: 2026-03-25 (급여대장 2행 레이아웃 리디자인 + 급여이력 직원수 버그 수정 + CA 감사 100%)
...
| 2026-03-25 | **급여대장 2행 레이아웃 리디자인** |
| 2026-03-25 | ├ 1인 1행 → 1인 2행: 인적사항(사번/이름/부서) \| 지급내역 \| 공제내역 \| 차인지급액 섹션 분리 |
| 2026-03-25 | ├ 2단 그룹 헤더 + splitHalf() 동적 항목 분배 (상단/하단 행) |
| 2026-03-25 | ├ 직원별 `<tbody>` 분리로 group hover 지원 + rowSpan=2 병합 (총지급/총공제/실수령) |
| 2026-03-25 | └ 섹션 구분선 (border-l-2) + 합계 행도 2행 구조 |
| 2026-03-25 | **급여이력 직원수 빈칸 버그 수정** |
| 2026-03-25 | └ API 응답 평탄화: employeeCount → totalEmployees 필드명 매핑 (SalaryCalcRepo↔프론트 불일치) |
| 2026-03-25 | **클린 아키텍처 전면 감사**: 123 API Route + 36 Domain + 102 Application 전수 검사 → **100% 준수** 재확인 |
