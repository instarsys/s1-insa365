...
async function handler(request: NextRequest, auth: AuthContext) {
  try {
    ...
    const { confirmPayrollUseCase } = getContainer();
    const result = await confirmPayrollUseCase.execute(auth.companyId, year, month, auth.userId, payrollGroupId);

    return successResponse(result);
  } catch (err) {
    const status = err instanceof ValidationError ? 400 : 500;
    return errorResponse(err instanceof Error ? err.message : '급여 확정 중 오류가 발생했습니다.', status);
  }
}
